...
if getNumPlanes() == 2 and getWidthByPlaneIdx(0) > 32 and getHeightByPlaneIdx(0) > 32:
    ...
    _luma = nvcv.as_image(luma)
    _luma_cuda = _luma.cuda()
    _chroma = nvcv.as_image(chroma)
    _chroma_cuda = _chroma.cuda()
    output = nvcv.as_image((_luma_cuda, _chroma_cuda), nvcv.Format.NV12)
